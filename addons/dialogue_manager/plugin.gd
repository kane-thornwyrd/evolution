...
func _enter_tree() -> void:
	...
	if Engine.is_editor_hint():
		...
		get_editor_interface().get_editor_main_screen().add_child.call_deferred(main_view)
		_make_visible(false)
...
func _copy_dialogue_balloon() -> void:
	...
	directory_dialog.get_vbox().add_child.call_deferred(label)
	...
	get_editor_interface().get_base_control().add_child.call_deferred(directory_dialog)
